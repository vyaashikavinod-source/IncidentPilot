...
describe("operator console routes", () => {
  ...
  it("renders dashboard incidents after an authenticated session", async () => { mockApi([{ body: list }]); render(<App/>); login(); expect(await screen.findByText("Gateway timeout")).toBeVisible(); expect(screen.getByText("Visible incidents: 1")).toBeVisible(); expect(screen.getByText("LOCAL")).toBeVisible(); expect(localStorage.length).toBe(0); });
